...
Когда на веб-сервере происходит подозрительный всплеск трафика или сканирование, инженеру нужно быстро получить локальную сводку и проверяемые строки из конечной выгрузки access-логов. SIGMA-PROBE делает это без постоянно работающего сервиса и передачи логов третьей стороне.

**SIGMA-PROBE создан для парадигмы «Один запуск — один сайт»:**

| Возможность | Как это устроено в SIGMA-PROBE |
| :--- | :--- |
| 🛡 **Zero-Dependency Architecture** | Работает на стандартной библиотеке Python 3.11+ без сторонних Python-пакетов для запуска и сборки. Безопасность Python и ОС проверяется отдельно. |
