...
Каждый новый bundle дополнительно содержит `manifest.json`: `version=1`, SHA-256 каждого включённого `report.json`, `report.html`, `report.txt` и, если задан отдельный `SIGMA_PROBE_REPORT_KEY` длиной не менее 32 байт UTF-8, `hmac_sha256` от канонических байтов манифеста без самого HMAC. `sigma-probe verify-report BUNDLE_DIR` проверяет файлы; для подписанного bundle передайте тот же ключ через переменную окружения. Без ключа манифест обнаруживает случайную порчу, но не удостоверяет происхождение: хеши и файл манифеста можно заменить вместе. HMAC обеспечивает проверку только сторонам, владеющим общим секретом; это не публичная цифровая подпись.
