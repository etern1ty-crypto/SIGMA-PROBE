...
def main() -> int:
    ...
    parser.add_argument('--scenario', choices=('repeated', 'unique'), default='repeated')
    args = parser.parse_args()
    if not 1 <= args.events <= 1_000_000 or not 1 <= args.actors <= min(args.events, 10_000):
        parser.error('events: 1..1000000; actors: 1..min(events,10000)')
    with tempfile.TemporaryDirectory() as directory:
        ...
        with path.open('w', encoding='utf-8') as handle:
            for index in range(args.events):
                ...
                path_part = '/health' if args.scenario == 'repeated' else f'/health/{index}'
                handle.write(f'2001:db8::{index % args.actors + 1:x} - - [{stamp}] "GET {path_part} HTTP/1.1" 200 128 "-" "curl/8.0"\n')
        started = time.perf_counter()
        result = AnalysisPipeline(Settings(limits=LimitsConfig(max_events=args.events))).run(str(path), write_reports=False)
