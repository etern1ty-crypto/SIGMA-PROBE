...
_PLAYBOOKS = (
    ...
    ('distributed-probing-review', {'DISTRIBUTED_PROBING'}, 'Проверить распределённые малочастотные пробы', (
        'Сверить совпадающие подозрительные пути, время и подсети; не считать разные IP доказательством общего оператора.',
        'Проверить доверенные proxy и законные распределённые проверки до изменения правил доступа.',
    )),
    ('web-shell-review', {'WEB_SHELL_TRAFFIC_CANDIDATE'}, 'Проверить возможный веб-шелл', (
        'Проверить путь в репозитории и на сервере, файловые изменения, error-логи и действия процесса приложения.',
        'Access-лог не содержит POST-body и не подтверждает выполнение команд или компрометацию.',
    )),
)
