...
An [Angie/Nginx JSONL logging example](examples/angie-nginx-log-format.conf) and [local Juice Shop lab](examples/lab/README.md) are available. A [GitHub Actions replay](https://github.com/etern1ty-crypto/SIGMA-PROBE/actions/runs/35846770795) passed with both real server binaries and tested the zipapp in Astra Linux, RED OS, and ALT Linux container images; Docker is unavailable in this local workspace. See [format compatibility](docs/COMPATIBILITY.md), [architecture](docs/ARCHITECTURE.md), [report schema](docs/REPORT_SCHEMA.md), [production limits](docs/PRODUCTION.md), and [validation notes](docs/VALIDATION.md).
...
For labeled local logs, [the evaluation script](docs/EVALUATION.md) computes actor-level precision, recall, and false-positive rate without writing incident reports. The notes include a small external Apache testbed result; the bundled labels are synthetic smoke data, not an independent accuracy claim.
